...
#[derive(Default)]
pub struct FtpsProtocolFactory;

impl FtpsProtocolFactory {
    pub fn new() -> Self {
        Self
    }
}
...
#[async_trait]
impl ProtocolFactory for FtpsProtocolFactory {
    ...
    async fn connect(&self, profile: &ConnectionProfile) -> OcResult<Box<dyn ProtocolSession>> {
        profile.validate()?;
        if profile.protocol != ProtocolKind::Ftps {
            return Err(OcError::InvalidProfile(
                "FTPS factory received a non-FTPS profile".to_owned(),
            ));
        }
        if !matches!(
            profile.security,
            ...
            return Err(OcError::InvalidProfile(
                "FTPS requires tls_explicit or tls_implicit security".to_owned(),
            ));
        }

        let password = profile.password.as_ref().ok_or_else(|| {
            OcError::InvalidProfile("FTPS requires password credentials".to_owned())
        })?;
        let tls = TlsConnector::builder()
            .build()
            .map_err(|err| OcError::Connection(format!("could not build TLS connector: {err}")))?;
        let tls = NativeTlsConnector::from(tls);
        let addr = profile.socket_addr();

        let mut stream = match profile.security {
            ConnectionSecurity::TlsExplicit => {
                let base = NativeTlsFtpStream::connect(addr.as_str()).map_err(map_ftps_error)?;
                base.into_secure(tls, profile.host.as_str())
                    .map_err(map_ftps_error)?
            }
            ConnectionSecurity::TlsImplicit => NativeTlsFtpStream::connect_secure_implicit(
                addr.as_str(),
                tls,
                profile.host.as_str(),
            )
            ...
            _ => {
                return Err(OcError::InvalidProfile(
                    "invalid FTPS security mode".to_owned(),
                ));
            }
        };
...
#[async_trait]
impl ProtocolSession for FtpsSession {
    fn kind(&self) -> ProtocolKind {
        ProtocolKind::Ftps
    }
    ...
    async fn list_dir(&mut self, path: &str) -> OcResult<Vec<RemoteEntry>> {
        ...
        self.stream.cwd(&normalized).map_err(map_ftps_error)?;
        self.cwd = normalized.clone();

        let lines = self.stream.list(None).map_err(map_ftps_error)?;
    ...
    async fn upload_file(&mut self, local_path: &str, remote_path: &str) -> OcResult<()> {
        ...
        self.stream.cwd(&remote_dir).map_err(map_ftps_error)?;
        self.cwd = remote_dir;
    ...
    async fn delete_path(&mut self, remote_path: &str) -> OcResult<()> {
        ...
        self.stream.rmdir(&target).map_err(map_ftps_error)?;
        Ok(())
    }
    ...
    async fn rename_path(&mut self, from: &str, to: &str) -> OcResult<()> {
        ...
        self.stream.rename(from, to).map_err(map_ftps_error)?;
        Ok(())
    }

    async fn disconnect(&mut self) -> OcResult<()> {
        self.stream.quit().map_err(map_ftps_error)?;
        Ok(())
    }
}

fn map_ftps_error(error: FtpError) -> OcError {
